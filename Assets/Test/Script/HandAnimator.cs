...
namespace MediaPipe.HandPose {
    ...
    public class HandAnimator : MonoBehaviour
    {
        ...
        [SerializeField] Transform _HandPos = null;
        ...
        void LateUpdate()
        {
            ...
            //print(_pipeline.GetKeyPoint(0) + ", " + HandPipeline.KeyPointCount);
            //var xformtest = CalculateJointXform(_pipeline.GetKeyPoint(0));
            //print(xformtest);
            float mux = 10f;
            float x = _pipeline.GetKeyPoint(0).x * (-1);
            float y = _pipeline.GetKeyPoint(0).y + 0.2f;
            float z = _pipeline.GetKeyPoint(0).z;
            Vector3 newPos = new Vector3(x, y, z) * mux;
            _HandPos.position = newPos;
            //var xform = CalculateJointXform(_pipeline.GetKeyPoint(0)* mux);
